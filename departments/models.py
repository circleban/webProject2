...
class Course(models.Model):
    ...
    type = models.CharField(max_length=10, 
                            choices=type_choices, default=th)
    ...
    def edit_course_url(self):
        return reverse('dept:editCourse', 
                       kwargs={'deptId': self.department.dept_id, 
                               'courseId': self.title})
    def delete_course_url(self):
        return reverse('dept:deleteCourse', 
                       kwargs={'deptId': self.department.dept_id, 
                               'courseId': self.title})
...
class Series(models.Model):
    ...
    running_semester = models.OneToOneField(Semester, 
                                            on_delete=models.CASCADE, 
                                            related_name='running_series')
    ...
    def get_absolute_url_control_panel(self):
        return reverse('dept:seriesControlPanel', 
                       kwargs={'deptId': self.department.dept_id, 
                               'serId': self.id})
    def get_absolute_url_course_allocation(self):
        return reverse('dept:courseTeacherAllocation', 
                       kwargs={'deptId': self.department.dept_id, 
                               'serId': self.id})
